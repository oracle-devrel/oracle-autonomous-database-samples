...
BEGIN
  ...
  ------------------------------------------------------------
  BEGIN
    DBMS_CLOUD_AI_AGENT.DROP_AGENT('ORACLE_AI_DATABASE_AGENT_ROLE');
    DBMS_OUTPUT.PUT_LINE('Dropped agent ORACLE_AI_DATABASE_AGENT_ROLE');
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('Agent ORACLE_AI_DATABASE_AGENT_ROLE does not exist, skipping');
  END;

    DBMS_CLOUD_AI_AGENT.CREATE_AGENT(
      agent_name => 'ORACLE_AI_DATABASE_AGENT_ROLE',
      attributes =>
        '{' || 
  ...
  DBMS_OUTPUT.PUT_LINE('Created agent ORACLE_AI_DATABASE_AGENT_ROLE');

  ------------------------------------------------------------
  -- DROP and CREATE TEAM
  ------------------------------------------------------------
  BEGIN
    DBMS_CLOUD_AI_AGENT.DROP_TEAM('ORACLE_AI_DATABASE_AGENT');
    DBMS_OUTPUT.PUT_LINE('Dropped team ORACLE_AI_DATABASE_AGENT');
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('Team ORACLE_AI_DATABASE_AGENT does not exist, skipping');
  END;
  ...
  DBMS_CLOUD_AI_AGENT.CREATE_TEAM(
      ...
      attributes =>
        '{' ||
        '"agents":[{"name":"ORACLE_AI_DATABASE_AGENT_ROLE","task":"ORACLE_AI_DATABASE_TASK"}],' ||
        '"process":"sequential"' ||
        '}',
      description =>
  ...
  DBMS_OUTPUT.PUT_LINE('Created team ORACLE_AI_DATABASE_AGENT');

  DBMS_OUTPUT.PUT_LINE('------------------------------------------------');
  DBMS_OUTPUT.PUT_LINE('Oracle AI Database Agent Team installation COMPLETE');
  DBMS_OUTPUT.PUT_LINE('------------------------------------------------');
