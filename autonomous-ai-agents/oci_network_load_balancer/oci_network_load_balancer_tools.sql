...
AS
  ...
  BEGIN
    ...
    l_cfg_json := get_agent_config(l_current_user, 'OCI_AGENT_CONFIG', c_nlb_agent);
  ...
  BEGIN
    ...
    IF l_credential_name IS NULL THEN
      l_result_json.put('status','error');
      l_result_json.put('message','Missing credential_name (set in OCI_AGENT_CONFIG for OCI_NETWORK_LOAD_BALANCER).');
      RETURN l_result_json.to_clob();
    END IF;
