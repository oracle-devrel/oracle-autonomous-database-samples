...
rem        - &&INSTALL_SCHEMA.oci_object_storage_agents
...
rem
rem      sqlplus admin@db @oci_object_storage_tools.sql <INSTALL_SCHEMA> [CONFIG_JSON]
rem
rem   3. Minimal install (uses defaults):
rem
rem      sqlplus admin@db @oci_object_storage_tools.sql <INSTALL_SCHEMA>
rem
...
• Automatic resolution of:
  ...
  - Compartment OCID
  - Credentials
  using centrally managed configuration (OCI_AGENT_CONFIG)
...
Configuration Model
-------------------
This package relies on a configuration table (OCI_AGENT_CONFIG) to store:
  - OCI credential name
