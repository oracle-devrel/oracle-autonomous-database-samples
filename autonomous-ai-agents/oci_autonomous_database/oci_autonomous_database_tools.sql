...
IS
  ...
  ----------------------------------------------------------------------------
  -- Apply config and persist in OCI_AGENT_CONFIG
  ----------------------------------------------------------------------------
...
AS
  ...
  BEGIN
    ...
    l_cfg_json := get_agent_config(l_current_user, 'OCI_AGENT_CONFIG', c_adb_agent);
  ...
  BEGIN
    ...
    IF l_credential_name IS NULL THEN
      l_result_json.put('status','error');
      l_result_json.put('message','Missing credential_name (set in OCI_AGENT_CONFIG for OCI_AUTONOMOUS_DATABASE).');
      RETURN l_result_json.to_clob();
    END IF;
    ...
    l_endpoint :=
      'https://identity.' || l_region || '.oci.oraclecloud.com/20160918/regionSubscriptions?tenancyId=' || tenancy_id;
